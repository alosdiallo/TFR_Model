...
```{r}
   ...
   iResample = 1
svmTuneRes = NULL
svmResult = svm(Train$cat~.,data=Train[,2:86],kernel="radial",cost=20,gamma = .1)
svmTestPred = predict(svmResult, newdata=Test[,2:86])
tmpVals = summPreds(predict(svmResult,newdata=Test[,2:86]),Test$cat)
dfTmp <- rbind(dfTmp,data.frame(resample=c("Validation","Bootstrap")[iResample],type="NB",metric=names(tmpVals),value=tmpVals))
dfTmpSVM <- rbind(dfTmpNB,data.frame(resample=c("Validation","Bootstrap")[iResample],type="NB",metric=names(tmpVals),value=tmpVals))
...
# logistic regression:
glmTrain <- glm(as.factor(Train$cat)~.,data=Train[,2:86],family=binomial)
glmTestPred <- predict(glmTrain, newdata=Test[,2:86], type="response") > 0.5
...
# LDA:
ldaTrain <- lda(Train$cat~.,data=Train[,2:86])
ldaTestPred <- predict(ldaTrain, newdata=Test[,2:86])
...
# NB:
Y = factor(as.numeric(as.factor(Train$cat)))
nbTrain <- naiveBayes(Y~.,data=Train[,2:86])
nbTestPred = predict(nbTrain, newdata=Test[,2:86])
...
for ( kTmp in c(10,20,150,300) ) {
  knnTestPred <- knn(Train[,2:86],Test[,2:86],Train$cat,k=kTmp)
...
ggsave(file="C:/Users/Alos Diallo/Documents/GitHub/TFR_Model/performance_plots.svg", plot = p, width=600, height=450, units="mm", dpi=900)
```
Quality Control
...
This is junk space for testing do not run
```{r}
    rfTestPred = NULL
    cNames = paste("col_", letters[1:20], sep="")
    cNames = c(cNames,"cat")
    cNames = as.character(cNames)
    train = sample(Train,20)
    train = cbind(train,Train$cat)
    colnames(train) = cNames
    test = sample(Test,20)
    test = cbind(test,Test$cat)
