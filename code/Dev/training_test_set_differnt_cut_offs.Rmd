...
---
Ths document is used to test different cutoffs for the training/test set.  You might have to run the code a few times to get it to work. This is for the supplimentry figure. 
...
```{r}
summPreds <- function(inpPred,inpTruth,inpMetrNms=c("err")) {
  retVals <- numeric()
...
dfTmpKNN = NULL
bTrain_ = NULL
bTrain_[1] = sample(1:nrow(model),round(0.9*nrow(model)),replace = TRUE)
bTrain_[2] = sample(1:nrow(model),round(0.8*nrow(model)),replace = TRUE)
bTrain_[3] = sample(1:nrow(model),round(0.7*nrow(model)),replace = TRUE)
bTrain_[4] = sample(1:nrow(model),round(0.6*nrow(model)),replace = TRUE)
bTrain_[5] = sample(1:nrow(model),round(0.5*nrow(model)),replace = TRUE)
names(bTrain_) = c(90,80,70,60,50)

for(i_sample in 1:5){
  for ( iSim in 1:50 ) {
    cols_to_use = 14:78
    i_sample = 4
    subset_of_rows = bTrain_[i_sample]
  #  bTrain = sample(1:nrow(model[1:subset_of_rows,]),round(0.7*nrow(model[1:subset_of_rows,])),replace = TRUE)
    ...
    Test$cat = as.numeric(as.factor(Test$cat))
      ...
      percentage = names(bTrain_[i_sample])
      train_title = paste(percentage,"Train",sep="+")
      test_title = paste(percentage,"Test",sep="+")
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="LR",metric=names(tmpVals),value=tmpVals))
    
      #logistic regression Training Error:
      glmTrain <- glm(as.factor(cat)~.,data=Train[,2:size],family=binomial)
      tmpVals <- summPreds(as.numeric(predict(glmTrain,type="response") > 0.5)+1,as.factor(Train$cat))
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="LR",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="LDA",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="LDA",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="NB",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="NB",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="SVM",metric=names(tmpVals),value=tmpVals))
      
      #SVM Training Error:
      svmResult = svm(factor(cat)~.,data=Train[,2:size],kernel="linear",cost = 1e+02) 
      svmTestPred = predict(svmResult, newdata=Test[,2:size])
      tmpVals = summPreds(as.numeric(predict(svmResult,newdata=Train[,2:size])),Train$cat)
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="SVM",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="RF",metric=names(tmpVals),value=tmpVals))
      
      #RF Training Error:
      rfTmp <- randomForest(factor(cat)~.,data=Train[,2:size],mTry = 10)
      rfTrainPred <- predict(rfTmp, newdata=Train[,2:size])
      tmpVals <- summPreds(as.numeric(rfTrainPred),Train$cat)
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="RF",metric=names(tmpVals),value=tmpVals))
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(test_title),type="KNN",metric=names(tmpVals),value=tmpVals))
      }
      ...
      dfTmp <- rbind(dfTmp,data.frame(resample=c(train_title),type="KNN",metric=names(tmpVals),value=tmpVals))
      }
